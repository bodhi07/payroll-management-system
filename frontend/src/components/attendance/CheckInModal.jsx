...
/**
 * Check-In Modal Form Component
 *
 * Logs employee check-in time and date for Member Module 2.
 */
const CheckInModal = ({ isOpen, onClose, onCheckIn }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      showToastSuccess('Check-in recorded successfully!');
      reset();
      onClose();
    } catch (err) {
      showToastError(err.response?.data?.message || 'Check-in failed. Employee may have already checked in today.');
    } finally {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Log Employee Check-In">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              type="time"
              step="1"
              {...register('checkInTime', { required: 'Time is required' })}
        ...
        <p className="text-[11px] text-[#737686]">
          * Note: Standard office start time is 08:30 AM. Check-ins after 08:30 AM will automatically compute late hours.
        </p>
